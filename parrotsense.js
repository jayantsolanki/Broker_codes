...
var log = new Logger({name:'Parrot-Sensor', 
         streams: [
    {
      level: 'info',
    ...
    {
      level: 'error',
      path: 'parrot.log'  // log ERROR and above to a file
    },
    {
      level: 'warn',
      path: 'parrot.log'  // log WARNING and above to a file
    },
    {
      level: 'info',
      path: 'parrot.log'  // log INFO and above to a file
    }
  ]

});
client.attachChannel(62, { writeKey:'VF9DGN3V5KNQRJ6F'});
console.log('Channel 62 attached');
var soilMoist, sunlightValue, airTemp, soilTemp, batValue;
...
/*
	...
	//soilTemp=temperature.toFixed(2);
      });
        ...
        async.series([
          function(callback) {
            ...
            flowerPower.readCalibratedSoilMoisture(function(error, soilMoisture) {
              log.info('calibrated soil moisture = ' + soilMoisture.toFixed(2) + '%');
	      soilMoist=soilMoisture.toFixed(2);
              callback();
            });
          },
          function(callback) {
            ...
            flowerPower.readCalibratedAirTemperature(function(error, temperature) {
              log.info('calibrated air temperature = ' + temperature.toFixed(2) + '°C');
	      airTemp=temperature.toFixed(2);
              callback();
            });
          },
          function(callback) {
            ...
            flowerPower.readCalibratedSunlight(function(error, sunlight) {
              log.info('calibrated sunlight = ' + sunlight.toFixed(2) + ' mol/m²/d');
	      sunlightValue=sunlight.toFixed(2);
              callback();
            });
          },
          function(callback) {
            ...
            flowerPower.readCalibratedEa(function(error, ea) {
              log.info('calibrated EA = ' + ea.toFixed(2));
          ...
          function(callback) {
            ...
            flowerPower.readCalibratedEcPorous(function(error, ecPorous) {
              console.log('calibrated EC porous = ' + ecPorous.toFixed(2) + ' dS/m');
              log.info("Battery Value "+batValue+"%, Soil Moist "+soilMoist+"%, Soil Temp  "+soilTemp+"C, Air Temp "+airTemp+"C, Sunlight "+sunlightValue);
              client.updateChannel(62, { "field1":batValue,"field2":airTemp,"field3":soilTemp,"field4":soilMoist,"field5":sunlightValue}, function(err, resp) {
                if (!err && resp > 0) {
                    log.info('Thingspeak feed update successfully for Parrot Sensor');
                    /*var jsonS={
                         "action":'thingspeak',
                         "data"  :"Thingspeak feed update successfully for channel id "+62;
                    };
