          ...
          log.error("Error in checking device entry in devices table"+err);
        	...
        	if(device.length>0){
              ...
              if(type==1){//primary battery for valves
                ...
                var sensorBat='UPDATE deviceNotif SET field1=\'0\' where deviceId in (SELECT device_id (SELECT device_id, field2 FROM feeds WHERE device_id=\''+deviceId+'\' ORDER BY id DESC LIMIT 1) as temp WHERE field2<\''+conditionValue+'\')';
                connection.query(query, function(err, sensorBat, fields) {
                  if (err) 
                    log.error("Error in checking feeds entry in devices table"+err);
                  else{
